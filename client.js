...
function setChatDisabled(isDisabled) {
    ...
    if (chatButton) 
        chatButton.disabled   = isDisabled;
...
async function saveUsername() {
    ...
    try {
        ...
        setChatDisabled(false);
        nameInput.placeholder = username;
        nameInput.value       = ""; alert(`Changed To: ${username}`);
    } catch (err) {
        console.error(err); alert("Error Saving Username");
    }
}
...
function handleIncomingchat(data) {
    ...
    while (allchats.length > 30) allchats[0].remove();
}
...
async function addchat() {
    ...
    const now = Date.now();
    if (now - lastchatTime < CHAT_COOLDOWN) return console.log("Wait a bit...");
    lastchatTime = now;
...
function initializeApp() {
    try {
        ...
        channel.subscribe('new-achievement', msg => handleIncomingchat(msg.data));
        initializeNameInput();
    } catch (error) {
